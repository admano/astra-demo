...
- [ ] **`process_pending_*()` has no per-call error handling.** In `ingestion.py`/`security.py`/`privacy/privacy.py`, a single unexpected row or transient SQLite error raises out of whichever thread called it (the 5s poll loop, or the `/notify` HTTP handler) and silently kills that path — the dashboard keeps responding (port stays "up") but new cases stop flowing through, with no visible error. Wrap the body in try/except + log, so a bad row can't quietly stall the pipeline mid-demo.
...
- [x] **Mixed import styles inside** `privacy/` — fixed. `anonymizer.py`, `scorer.py`, and `privacy/privacy.py` used unqualified sibling imports (`from vault import vault`), which loaded `vault`/`models`/`detector` as second module objects distinct from the qualified `privacy.vault` etc. used by `pipeline.py`/`detector.py`. That meant **two separate `PseudonymVault()` singletons** — pseudonyms got written into one instance, `tokens_found`/`pii_tokens` got read from the other, so the Privacy dashboard's token count and token table were silently always empty even though anonymization itself worked correctly. All sibling imports are now qualified (`from privacy.X import ...`) everywhere, including the `privacy.py` entry script — see `CLAUDE.md` for the why.
